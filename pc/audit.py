...
def 살피기(창고: Notes) -> dict[str, list]:
    ...
    돌려주는 것(빈 목록이면 성한 것):
      ...
      `겹친요약`  [(원본, [쪽들])]          — 한 원본을 제 원본이라 밝힌 쪽이 둘 이상
    ...
    # ★★ **한 원본에 요약이 둘.** 실제로 두 번 났다(2026-09-21). VC 는 원본이 들어오면
    #   뒤에서 스스로 합치는데, 사람이 **몇 분 뒤에** 제대로 된 쪽을 쓰면 이미 VC 것이
    #   만들어진 뒤다 — 먼저 만든 쪽을 안 지우니 창고에 같은 말이 둘 남는다.
    #   막을 수가 없는 **경주**라, 합치기를 미루는 대신 **난 뒤에 잡는다.**
    #   ★ 「가리키기만」 하는 것은 안 센다 — 엮음 글의 「읽은 것」 목록이 그렇다.
    #     `원본:` 줄로 **제 원본이라고 밝힌** 쪽만 요약으로 본다.
    선언: dict[str, list[str]] = {}
    for 제목, 글 in 글들.items():
        if 층of[제목] != wiki.WIKI:
            continue
        for 이름 in synth.원본선언(글.body):
            닿은 = 풀기(이름)
            if 닿은 and 층of.get(닿은) == wiki.RAW:
                선언.setdefault(닿은, []).append(제목)
    #   ★★ **둘이라고 다 겹친 것이 아니다.** 원본 하나에서 제대로 된 쪽이 둘 나오는 것은
    #     정상이다(Cookbook 하나로 「기억과 접기」·「크게 재기」를 따로 썼다).
    #     겹친 것은 **VC 가 지은 `… 요점` 옆에 딴 쪽이 선** 때다 — 요점은 기계의 첫 손이라
    #     제대로 된 쪽이 서면 자리를 비켜야 한다. 그 경우만 잡는다.
    겹친요약 = []
    for 원본, 쪽들 in sorted(선언.items()):
        if synth.요약제목(원본) in 쪽들 and len(쪽들) > 1:
            겹친요약.append((원본, sorted(쪽들)))
...
def _self_check() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # ★★ **한 원본에 요약이 둘.** VC 가 뒤에서 합친 뒤에 사람이 제대로 된 쪽을 쓰면
        #   같은 말이 둘 남는다 — 막을 수 없는 **경주**라 난 뒤에 잡는다(2026-09-21 두 번 났다).
        assert 살피기(창고)["겹친요약"] == [], 살피기(창고)["겹친요약"]
        창고.write(Note(title="사람이 쓴 요약", body="더 낫다.\n\n원본: [[원본 가]]", kind="개념"))
        assert 살피기(창고)["겹친요약"] == [("원본 가", ["사람이 쓴 요약", "원본 가 요점"])], \
            살피기(창고)["겹친요약"]
        # ★ **가리키기만** 하는 것은 요약이 아니다 — 엮음 글의 「읽은 것」 목록이 그렇다.
        #   안 가르면 엮음 하나가 원본 여섯을 「내가 요약했다」고 주장한다.
        창고.write(Note(title="엮음", body="## 읽은 것\n\n[[원본 가]] · [[원본 나]]", kind="설계"))
        assert 살피기(창고)["겹친요약"] == [("원본 가", ["사람이 쓴 요약", "원본 가 요점"])], \
            "가리키기만 한 엮음을 요약으로 셌다"
        창고.delete("엮음")
        # ★★ **둘이라고 다 겹친 것이 아니다.** `… 요점` 이 없으면 원본 하나에서 제대로 된
        #   쪽이 둘 나온 것 — 정상이다. 이것까지 잡으면 사람이 알림을 안 보게 된다.
        창고.write(Note(title="사람이 쓴 요약 둘", body="딴 결.\n\n원본: [[원본 나]]", kind="개념"))
        assert 살피기(창고)["겹친요약"] == [("원본 가", ["사람이 쓴 요약", "원본 가 요점"])], \
            f"제대로 된 쪽 둘을 겹쳤다고 한다: {살피기(창고)['겹친요약']}"
