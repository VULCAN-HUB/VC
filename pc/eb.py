...
def main(argv: list[str] | None = None) -> int:
    ...
    # ★★ **켜져 있는 동안에도 본다.** 켤 때 한 번만 보면, 며칠씩 켜 두는 사람은
    #   새 판이 나와도 영영 모른다(오너 2026-09-26). 세 시간에 한 번 — 자주 물어
    #   깃허브를 두들길 일도, 하루를 놓칠 일도 아니다.
    #   ★ 못 물어봐도 조용히 넘어간다. 업데이트 확인이 프로그램보다 중하면 안 된다.
    win._새판시계 = _때알림(win)
    win._새판시계.timeout.connect(lambda: win.새판찾기())
    win._새판시계.start(3 * 60 * 60 * 1000)
    if 화면상태:
        _화면상태재기(app, win)
    code = app.exec_()
...
def _시험표(argv: list[str]) -> int:
    ...
    def 진단(글: str) -> str:
        d = json.loads(글)
        # ★ **판을 맨 앞에 적는다.** 시험표에 판이 **파일 이름에만** 있어서, 표를 옮겨
        #   적거나 한 줄만 인용하면 **어느 판을 잰 것인지 사라진다.** 시험 쪽 보고에서
        #   실제로 그랬다 — `--doctor` 는 판을 찍고 있는데 표가 그 칸을 안 골랐다.
        return (f"판 {d.get('판')} · 항목 {d.get('항목 수')} · 연결 {d.get('연결 수')}"
                f" · 판 적합률 {d.get('판 적합률')}"
...
def _self_check() -> None:
    ...
    #   ※ 세는 글자는 **설명 문장에 안 쓴다** — 처음엔 이 주석에도 같은 말을 적었더니
    #     진짜 줄을 지워도 두 군데가 남아 안 터졌다.
    for 있어야, 몇, 까닭 in (("AI 가 붙는 법", 2, "AI 가 어디로 붙는지 안 알린다"),
                          ("의 pair_token", 2, "열쇠가 어느 파일에 있는지 안 알린다")):
        assert not 소스있다 or 본문3.count(있어야) >= 몇, f"{까닭} ({본문3.count(있어야)}군데)"
    ...
    본문2 = _소스글()
    # ★★ **`in` 으로 세면 검사가 제 몸을 본다.** 찾는 글자가 이 검사문 안에 리터럴로
    #   들어 있어 **진짜 코드를 지워도 늘 참**이었다 — 둘 다 아무것도 안 막고 있었다.
    #   개수로 센다(진짜 코드 한 번 + 이 검사문 한 번이라 **둘**이 바닥이다).
    for 있어야, 까닭 in ((chr(39) + '정답칸.split(";")' + chr(39))[1:-1], "정답을 여럿 못 받는다"),                        ("min(자리) if 자리 else 0", "여럿 중 제일 위를 안 센다"):
        assert not 본문2 or 본문2.count(있어야) >= 2, f"{까닭} ({본문2.count(있어야)}군데)"
    ...
    for 있어야, 몇번, 까닭 in (
            ...
            # 벡터를 채우는 길이 화면에만 있으면, 재려고 창을 띄워 두고 기다려야 한다.
            ("while n.embed_some(64):", 2, "찾기점수가 못 만든 벡터를 스스로 안 채운다")):
        assert not 본문2 or 본문2.count(있어야) >= 몇번, f"{까닭} ({본문2.count(있어야)}군데)"

    # (이 글자들은 이 파일에서 `--doctor` 자리에만 있다 — 진단 묶음 쪽은 report.py 다)
    본문 = _소스글()
    # ★ 판을 밝히는 줄이 빠지면 시험하는 쪽이 **받은 것이 어느 판인지** 못 잰다.
    # ★★ **`in` 으로 세면 검사가 제 몸을 본다.** 이 표는 원래 `있어야 in 본문` 이었는데,
    #   찾는 글자가 **이 검사문 안에 리터럴로 들어 있어** 진짜 코드를 지워도 참이었다 —
    ...
    # ★ 시험표의 `--doctor` 줄은 **판부터** 적는다. 표를 옮겨 적거나 한 줄만 인용하면
    #   파일 이름에 있던 판이 사라져 **어느 판을 잰 것인지** 없어진다(시험 쪽 보고가 그랬다).
    #   진짜 코드 한 번 + 이 검사문 한 번이라 **둘**이 바닥이다.
    #   ★ 세는 글자에 따옴표를 넣지 마라 — 검사문이 그걸 `\"` 로 담아서 **제 몸을 못 센다**
    #     (그래서 코드가 멀쩡한데 1군데로 세어 터졌다). 이스케이프 없는 조각으로 센다.
    #     조건과 메시지는 **같은 글자**를 세야 한다 — 다르면 숫자가 거짓말을 한다.
    있어야 = "판 {d.get"
    assert not 본문 or 본문.count(있어야) >= 2, \
    ...
    #   ★ 글자를 세지 않고 **실제로 불러 본다** — 이름만 바꿔도 통과하는 검사는 죽은 검사다.
...
if __name__ == "__main__":
    ...
    if "--찾기점수" in sys.argv or "--score" in sys.argv:
        ...
        # ★★ **벡터를 여기서 채운다.** 전에는 못 만든 벡터를 **세기만** 했다.
        # 그러면 벡터가 빈 창고에서 낱말 검색만 잰 값이 나오는데, 숫자는 그럴듯해서
        # 「물건이 못 찾는다」로 읽힌다 — 창고를 사본으로 옮겨 재던 자리가 그랬다.
        # 채우는 길이 **화면에만** 있어서 재려면 창을 띄워 두고 기다려야 했다.
        # 재는 도구가 제 재료를 스스로 갖춘다. 이미 있으면 한 바퀴에 끝난다.
    ...
    if "--진단" in sys.argv or "--doctor" in sys.argv:
        ...
        try:
            ...
            # ★ **판을 먼저 밝힌다.** 구운 판은 판 번호를 어디에도 안 찍었다 —
            #   `--doctor` 에도, `version.txt` 에도, 기록에도 없었다. 그래서 시험하는 쪽이
            #   **받은 것이 어느 판인지** sha256 을 대조하는 길밖에 없었다.
            #   (내가 이 판을 내보내기 직전에 「이게 정말 새 판인가」를 못 재서 걸린 자리다.
            #    `--시험표` 파일 이름으로 겨우 확인했다.)
            report["판"] = f"v{paths.VERSION}"
            ...
            # ★★ **이 창고의 마지막 쓸모는 AI 가 꺼내 쓰는 것이다.** 그런데 붙는 법이
            #   어디에도 안 적혀 있어, 같은 PC 에서 도는 AI 도 포트와 토큰 자리를 사람에게
            #   물어야 했다. 여기 한 줄이면 `--doctor` 만 보고 바로 붙는다.
            #   **토큰 값은 안 적는다** — 열쇠를 파일 두 곳에 두지 않는다. 자리만 말한다.
            # ★★ **하다 만 이름 바꾸기는 그래프가 반쯤 끊긴 상태다.** 쪽지가 남아 있으면
            #   일부 파일만 고쳐진 것이라 링크가 허공을 가리킨다. 지금까지는 **창만** 그것을
            #   봤다 — `--no-ui` 로 서버만 띄워 쓰면 아무도 모른 채 계속 돈다.
            하다만 = n.이름바꾸다만것()
            if 하다만:
                report["하다 만 이름 바꾸기"] = (
            ...
            # ★ **더 잘 찾는 길이 있으면 알려 준다.** 큰 뜻 모델(e5-base)을 받으면
