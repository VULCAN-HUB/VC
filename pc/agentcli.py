...
def 돌리기(프로젝트: str, 지시: str, 손이름: str = "claude", 제한초: int = 기본제한초,
    ...
    """
    지시 = (지시 or "").strip()
    # ★★ **프로젝트 없이도 시킬 수 있어야 한다.** 「창고에 적어 둬」처럼 코드와
    #   상관없는 일이 있다. 창은 일터 자리를 넘겨 주는데 **문(HTTP)은 안 넘겨서**,
    #   굽힌 앱에 시키니 「그런 프로젝트가 없다」로 끝났다(실기로 잡았다 · 2026-09-24).
    #   자리를 푸는 곳이 여기 하나뿐이니 **여기서** 푼다 — 부르는 자리마다 넘기면
    #   또 한 군데를 빠뜨린다.
    일터이름 = paths.일터().name
    if 뿌리 is None and (not 프로젝트 or 프로젝트 == 일터이름):
        프로젝트, 뿌리 = 일터이름, paths.일터().parent
...
def 사람말(난것: dict) -> str:
    ...
    if 난것.get("왜") and not 난것.get("끝난코드"):
        return f"못 돌렸어 — {난것['왜']}"
    바뀐 = 난것.get("바뀐파일") or []
    잰것 = (f"{난것.get('손')} 가 {난것.get('든시간', 0)}초 돌았어 "
          f"({'됐다' if 난것.get('됐나') else '탈났다'}"
          + (f" — {난것['왜']}" if 난것.get("왜") else "") + ")")
    # ★★ **고친 파일이 없다고 아무 일도 안 한 것이 아니다.** 창고 일은 도구로 하니
    #   git 에는 아무것도 안 남는데, 「바뀐 파일 0개 — 없다」를 앞세웠더니 **실패한
    #   것처럼 읽혔다**(실기로 보고 고쳤다 · 2026-09-24). 그럴 때는 **제가 한 말**을
    #   앞에 세운다 — 그게 사람이 알고 싶은 것이다.
    말 = (난것.get("읽을말") or 난것.get("끝말") or "").strip()
    if not 바뀐:
        한줄 = 잰것 + ((" — " + 말.splitlines()[0][:200]) if 말 else "")
        머리 = [한줄]
        if 난것.get("원래더럽던것"):
            머리.append(f"★ 돌리기 전부터 고쳐져 있던 것 {len(난것['원래더럽던것'])}개 — "
                      f"되돌릴 때 같이 날리지 않게 조심: "
                      f"{' · '.join(난것['원래더럽던것'][:5])}")
        return "\n".join(머리)
    머리 = [잰것]
    머리.append(f"바뀐 파일 {len(바뀐)}개: {' · '.join(바뀐[:5])}")
    if 난것.get("원래더럽던것"):
        머리.append(f"★ 돌리기 전부터 고쳐져 있던 것 {len(난것['원래더럽던것'])}개 — "
...
def _self_check() -> None:
    ...
    # ★ 이 시험은 **셸 스크립트 하나를 띄워** 잰다. 윈도우에는 `#!/bin/sh` 가 없어
    #   여기서는 건너뛴다 — **없어서 못 재는 것과 재서 틀린 것은 다른 말이다.**
    #   (윈도우는 창으로 켜도 PATH 를 물려받아 이 병 자체가 안 난다.)
    def _길시험(_길tmp: str) -> None:
        가짜빈 = Path(_길tmp) / "bin"
        가짜빈.mkdir()
        놈 = 가짜빈 / "vc-시험-클리"
        놈.write_text("#!/bin/sh\necho '{\"result\":\"길 시험\"}'\n", encoding="utf-8")
    ...
    if os.name != "nt":
        with tempfile.TemporaryDirectory() as _길자리:
            _길시험(_길자리)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # ★★ **고친 것이 없어도 실패처럼 보이면 안 된다** — 창고 일은 git 에 안 남는다
        난것 = 돌리기("CliApp", "가만히 있어라", 뿌리=뿌리, 손물건=가짜())
        말한것 = 사람말(난것)
        assert "0개" not in 말한것 and "없다" not in 말한것.split("\n")[0], 말한것
        assert "가짜 손이 돌았다" in 말한것, 말한것
        ...
        # ★★ **프로젝트 없이도 시킬 수 있어야 한다** — 창고 일은 코드와 상관없다.
        #   창은 일터를 넘겨 주는데 문은 안 넘겨서 굽힌 앱이 「그런 프로젝트가 없다」로
        #   끝났다(실기 · 2026-09-24). 자리 푸는 곳 한 군데에서 푼다.
        for _빈것 in ("", paths.일터().name):
            _난것빈 = 돌리기(_빈것, "일해라", 손물건=가짜())
            assert _난것빈.get("왜") != "그런 프로젝트가 없다", (_빈것, _난것빈)
            assert _난것빈["됐나"], (_빈것, _난것빈)
        # 뿌리를 준 것은 그대로 둔다 — 넘겨 준 자리를 덮으면 안 된다
