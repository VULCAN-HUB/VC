...
def 협업(창고: Notes, 프로젝트: str, 지시: str, 짓는손: str = "claude",
    ...
    지음 = agentcli.돌리기(프로젝트, 지시, 짓는손, 뿌리=뿌리, 멈춤=멈춤,
                       손물건=짓는물건, **({"제한초": 제한초} if 제한초 else {}))
    잴것 = {"지음": 지음, "봄": {}, "적립": {"만든것": []}, "스킬제안": {}}

    바뀐 = 지음.get("바뀐파일") or []
    if 지음.get("됐나") and 바뀐:
        말 = 리뷰틀.format(지시=지시, 파일들=" · ".join(바뀐[:10]),
                        차이=(지음.get("차이") or "")[:20000])
        잴것["봄"] = agentcli.돌리기(프로젝트, 말, 보는손, 뿌리=뿌리, 멈춤=멈춤,
                                손물건=보는물건, 읽기전용=True,
                                **({"제한초": 제한초} if 제한초 else {}))

    난것 = 돌린뒤(창고, 프로젝트, 지시, 지음)
    잴것["적립"], 잴것["스킬제안"] = 난것.get("적립") or {"만든것": []}, 난것.get("스킬제안") or {}
    ...
    if 본것 and (잴것["적립"].get("만든것") or []):
        ...
        글 = 창고.read(제목)
        if 글 is not None:
            말 = (본것.get("나온말") or "").strip() or (본것.get("끝말") or "")
...
def _self_check() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp둘:
        ...
        창고둘.reindex()
        제목둘 = 난것["적립"]["만든것"][0]
        assert "가 본 것" in (창고둘.read(제목둘).body or ""), 창고둘.read(제목둘).body[-200:]
